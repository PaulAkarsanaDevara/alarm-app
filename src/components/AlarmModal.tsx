...
const SOUNDS: AlarmSound[] = ['gentle', 'classic', 'digital', 'birds']
...
export default function AlarmModal() {
  ...
  function handleSave() {
    if (!time) return
    if (editingAlarm) {
      dispatch(updateAlarm({ ...editingAlarm, time, label, repeat, sound }))
    } else {
      dispatch(addAlarm({ time, label, repeat, sound, enabled: true }))
    }
    dispatch(closeModal())
  }
